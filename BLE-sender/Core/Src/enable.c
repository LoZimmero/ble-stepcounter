...
#define THRESHOLD 0.7  		// threshold value for peak detection
...
void ble_init(){
	...
	//add the second charachteristic, it's the analog value coming from the microphone
	addCharacteristic(UUID_CHAR_2,MIC_CHAR_HANDLE,CUSTOM_SERVICE_HANDLE,SET_CONTENT_LENGTH(22),READABLE|NOTIFIBLE);

	//setchar2
	updateCharValue(CUSTOM_SERVICE_HANDLE,MIC_CHAR_HANDLE,0,SET_CONTENT_LENGTH(22),VALUE2);
	...
	addService(UUID_GYROSCOPE_SERVICE,GYROSCOPE_SERVICE_HANDLE,1+2+3*3);

	//add name service
	addCharacteristic(UUID_CHAR_GYROSCOPE_NAME,NAME_GYROSCOPE_HANDLE,GYROSCOPE_SERVICE_HANDLE,SET_CONTENT_LENGTH(20),READABLE);

	//set name
	updateCharValue(GYROSCOPE_SERVICE_HANDLE,NAME_GYROSCOPE_HANDLE,0,SET_CONTENT_LENGTH(20),NAME_GYROSCOPE_VALUE);


	//add gyro axis x
	addCharacteristic(UUID_CHAR_GYROSCOPE_GYROX,GYROX_CHAR_HANDLE,GYROSCOPE_SERVICE_HANDLE,SET_CONTENT_LENGTH(18),READABLE|NOTIFIBLE);

	//set gyro axis x

	updateCharValue(GYROSCOPE_SERVICE_HANDLE,GYROX_CHAR_HANDLE,0,SET_CONTENT_LENGTH(18),X_VALUE);


	//add gyro axis y
	addCharacteristic(UUID_CHAR_GYROSCOPE_GYROY,GYROY_CHAR_HANDLE,GYROSCOPE_SERVICE_HANDLE,SET_CONTENT_LENGTH(18),READABLE|NOTIFIBLE);

	//set gyro axis y

	updateCharValue(GYROSCOPE_SERVICE_HANDLE,GYROY_CHAR_HANDLE,0,SET_CONTENT_LENGTH(18),Y_VALUE);

	//add gyro axis z
	addCharacteristic(UUID_CHAR_GYROSCOPE_GYROZ,GYROZ_CHAR_HANDLE,GYROSCOPE_SERVICE_HANDLE,SET_CONTENT_LENGTH(18),READABLE|NOTIFIBLE);

	//set gyro axis z

	updateCharValue(GYROSCOPE_SERVICE_HANDLE,GYROZ_CHAR_HANDLE,0,SET_CONTENT_LENGTH(18),Z_VALUE);
...
int fetchBleEvent(uint8_t *container, int size){
  ...
  HAL_SPI_TransmitReceive(&hspi2,master_header,slave_header,5,1);
  ...
  if(dataSize>0){
	    //let's fill the get the bytes availables and insert them into the container variable
  		...
  		HAL_SPI_TransmitReceive(&hspi2,(uint8_t*)&dummy,container+i,1,1);
...
void sendCommand(uint8_t *command,int size){
	...
	if(bufferSize>=size){
		HAL_SPI_Transmit(&hspi2,command,size,1);
		result=0;
	}else{
		result=-1;
	}
...
// Step counter algorythm

//int step_count = 0;
int check_step(int16_t x, int16_t y, int16_t z, int* step_count) {
	double magnitude, threshold_value;
    int peak_detected = 0;

    magnitude = compute_magnitute(x, y, z);

    // calculate threshold value
    threshold_value = THRESHOLD/* * (float)(magnitude[0])*/;
    // detect peaks in the magnitude signal
	if (magnitude > threshold_value) {
		peak_detected = 1;
	}
	// count steps when a valley is detected
	if (peak_detected && magnitude < threshold_value) {
		peak_detected = 0;
		(*step_count)++;
	}
    return 0;
...
double compute_magnitute(int x, int y, int z) {
